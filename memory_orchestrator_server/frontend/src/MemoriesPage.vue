    ...
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        ...
        <button @click="load" class="btn-refresh" :class="{ loading: isLoading }" :title="t('Refresh')">
          <IconRefresh width="14" height="14" :class="{ spinning: isLoading }" />
          {{ t('Refresh') }}
        </button>
        <span class="toolbar-sep"></span>
      ...
      <table>
        ...
        <thead>
          <tr>
            ...
            <th><span class="th-inner"><IconFolder width="11" height="11" />{{ t('Project') }}</span></th>
            <th class="type-col"><span class="th-inner"><IconTag width="11" height="11" />{{ t('Type') }}</span></th>
            <th><span class="th-inner"><IconTerminal width="11" height="11" />{{ t('Name') }}</span></th>
            <th><span class="th-inner"><IconAlignLeft width="11" height="11" />{{ t('Description') }}</span></th>
            <th class="sortable col-hits" @click="toggleSort('hits')">
              <span class="th-inner"><IconActivity width="11" height="11" />{{ t('Hits') }}<span class="sort-icon" v-if="sortBy === 'hits'">{{ sortDesc ? '↓' : '↑' }}</span></span>
            </th>
            <th class="sortable col-updated" @click="toggleSort('time')">
              <span class="th-inner"><IconClock width="11" height="11" />{{ t('Updated') }}<span class="sort-icon" v-if="sortBy === 'time'">{{ sortDesc ? '↓' : '↑' }}</span></span>
            </th>
          ...
          <tr v-if="paged.length === 0">
            <td colspan="9" class="empty">
              <div class="empty-inner">
                <IconDatabase width="36" height="36" style="opacity:0.25" />
    ...
    <div v-if="loginOpen" class="modal-overlay login-overlay">
      <div class="login-modal">
        <div class="login-logo">
          <img :src="logoUrl" width="28" height="28" alt="Memory Orchestrator logo" />
        ...
        <form class="login-form" @submit.prevent="submitLogin">
          ...
          <div class="login-actions">
            <button class="login-btn" type="submit" :disabled="!loginInput || loginLoading">
              ...
              <IconLogout v-else width="13" height="13" />
              {{ t('Sign in') }}
            </button>
          </div>
        </form>
        ...
        <div v-else-if="settingsTab === 'scoring'" class="modal-body scoring-body">
          ...
          <div class="settings-group">
            ...
            <div v-if="!weightSumOk" class="score-warn">
              <IconWarn width="12" height="12" /> {{ t('Weights should sum to 1.0') }} ({{ t('current') }}: {{ weightSum }})
            </div>
          </div>
    ...
    <div v-if="deleteTarget" class="modal-overlay">
      <div class="modal modal-sm">
        ...
        <div class="modal-body delete-modal-body">
          <IconTrash width="32" height="32" class="delete-icon" />
        ...
        <div class="write-body">
          ...
          <div class="write-collapsible" :class="{open: writeShowExtra}">
            <button class="write-extra-toggle" @click="writeShowExtra = !writeShowExtra">
              <IconChevron :class="['write-extra-chevron', {open: writeShowExtra}]" width="10" height="10" />
              {{ t('Optional fields') }}
            </button>
...
import IconClone from './icons/IconClone.svg'
...
import IconDuplicates from './icons/IconDuplicates.svg'
import IconConflicts from './icons/IconConflicts.svg'
import IconFolder from './icons/IconFolder.svg'
import IconTag from './icons/IconTag.svg'
import IconTerminal from './icons/IconTerminal.svg'
import IconAlignLeft from './icons/IconAlignLeft.svg'
import IconActivity from './icons/IconActivity.svg'
import IconClock from './icons/IconClock.svg'
import IconDatabase from './icons/IconDatabase.svg'

...
```python
hits = hits[:top_k]   # 默认 top_k=3，可在 Settings → search_top_k 修改
```
...
kept = truncate_by_budget(items, budget=hook_budget_tokens)  # 默认 2000 tokens
```
...
| key | 默认值 | 说明 |
|-----|--------|------|
| `search_top_k` | 3 | 最终返回条数；粗排取 top_k×3 候选（即 9 条） |
| `hook_budget_tokens` | 2000 | 注入到系统提示的 token 上限 |
| `rerank_enabled` | "false" | "true" 开启 BGE-reranker 精排 |
| `rerank_model` | bge-reranker-v2-m3 | 精排模型路径（仅信息显示） |
| `hook_cooldown_sec` | 300 | UserPromptSubmit 最短触发间隔（秒） |
| `hook_min_turns` | 1 | 至少 N 轮新用户对话后才触发 context 注入 |
| `dup_threshold` | 0.92 | 保存时查重的余弦相似度阈值 |
| `score_type_feedback` | 1.3 | feedback 类型排名乘数 |
| `score_type_project` | 1.1 | project 类型排名乘数 |
| `score_type_user` | 1.0 | user 类型排名乘数 |
| `score_type_reference` | 0.9 | reference 类型排名乘数 |

...
# apt-get 不配置 SOCKS5（不支持），仅对 git clone 生效
...
- **图名**：`memory_graph`（单图，所有 project 共享图结构，但查询始终按 `project_id` 过滤）
- 在 Alembic migration `0010_age_extension.py` 中初始化
- **不含全局记忆**：`project_id = '00000000-0000-0000-0000-000000000000'`（slug `*`）的记忆不写入图，图推理仅在同一 project 内进行
...
每次 `save_memory` / `patch_memory` 后同步 upsert 顶点（跳过全局 project）。`delete_memory` 时删除顶点及其所有边。
...
- `memory_links` 表在 migration `0010_age_extension.py` 中**直接删除**（`DROP TABLE memory_links`）
- AGE 完全承担图职责；AGE 顶点 `mem_id` 与 `memories.id` 对应，无跨表 FK 约束（AGE 不支持）
...
```
save_memory(M) 提交成功
  → 若 M.project_id == GLOBAL_PROJECT_ID: 跳过所有图操作
  → graph.sync_vertex(M)           # upsert AGE 顶点
  → candidates = search(M, project_id=M.project_id, top_k=20)  # 同项目向量召回
...
```python
# 对每个命中，查询 graph_hop_depth 跳邻居（project_id 过滤保证项目隔离）
MATCH (m:Memory {mem_id: $id})-[r*1..{depth}]-(n:Memory {project_id: $project_id})

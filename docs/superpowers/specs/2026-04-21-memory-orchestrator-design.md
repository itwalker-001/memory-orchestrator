...
| Embedding | 本地 bge-m3 | OpenAI / Voyage / 可插拔 | 中文效果好、离线、无 API 成本 |
...
│             ▼                              ▼                │
│   ┌───────────────────┐          ┌───────────────────┐      │
│   │ PostgreSQL 16     │          │  bge-m3 embedding │      │
│   │ + pgvector        │          │  (FastEmbed/本地) │      │
│   └───────────────────┘          └───────────────────┘      │
└─────────────────────────────────────────────────────────────┘
```
...
3. **数据层**:PostgreSQL 负责元数据 + 向量,bge-m3 跑在 orchestrator 进程内
...
1. bge-m3 1024 维索引在 HNSW 下的 m / ef_construction 参数调优 — 先用 pgvector 默认值,有性能问题再调

...
from memory_orchestrator.client_rules import (
...
@click.option("--client", type=click.Choice(["claude", "codex"]), default=None,
              help="Override MO_CLIENT env var; baked into launch args by mo setup.")
...
def _install_claude(scope: str) -> None:
    ...
    try:
        mcp_result = subprocess.run(
            ["claude", "mcp", "add", "--scope", mcp_scope, "memory-orchestrator",
             "--", "uv", "run", "--no-sync", "--project", str(project_dir), "mo", "serve-mcp", "--client", "claude"],
    ...
    if mcp_result is not None and mcp_result.returncode == 0:
        ...
        click.echo(f"mcp add failed (run manually): claude mcp add --scope {mcp_scope} memory-orchestrator -- uv run --no-sync --project {project_dir} mo serve-mcp --client claude")
